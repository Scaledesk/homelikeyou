...
class Mdl_hlu_email_settings extends CI_Model{
...
//        die;
        ...
        $this->setSmtpPass($settings[0]['hlu_email_settings_smtp_pass']);
        $this->setPort($settings[0]['hlu_email_settings_port']);
        }else{
            $this->setSmtpHost('');
            $this->setSmtpUser('');
            $this->getSmtpPass('');
    ...
    public function update(){
        if($this->db->where('hlu_email_settings_id',self::ID)->select(array('hlu_email_settings_id'))->get('hlu_email_settings')->result_array()){
        return $this->db->where('hlu_email_settings_id',self::ID)->update('hlu_email_settings',[
            'hlu_email_settings_smtp_host'=>$this->getSmtpHost(),
            'hlu_email_settings_smtp_user'=>$this->getSmtpUser(),
            'hlu_email_settings_smtp_pass'=>$this->getSmtpPass(),
            'hlu_email_settings_port'=>$this->getPort()
        ])?true:false;}
        else{
    ...
    public function insert(){
        return $this->db->insert('hlu_email_settings',[
            'hlu_email_settings_smtp_host'=>$this->getSmtpHost(),
            'hlu_email_settings_smtp_user'=>$this->getSmtpUser(),
            'hlu_email_settings_smtp_pass'=>$this->getSmtpPass(),
            'hlu_email_settings_port'=>$this->getPort()
